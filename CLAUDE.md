...
The leads you find and enrich live in a database you own, not a vendor tool you rent. The recommended home is the user's own **Supabase Postgres**, wired in as an MCP, with the schema in `supabase/schema.sql`. It is modeled on Clay's waterfall enrichment structure, so it is genuinely a Clay replacement the user owns: `companies` (firmographics enriched once), `leads` (people attached to a company, with per-field provenance on email and phone), and `enrichment_events` (a log of every provider call, so you can see which one won and what it cost). The ICP score can come from Nous (call the ICP model, one Nous workspace per client for agencies) or a scoring skill, and is always visible in the `lead_list_overview` view. `/onboard` sets it up (Supabase, or Airtable, Sheets, or a CSV as fallbacks); `connections.md` records which one is configured. Render it as a spreadsheet with `references/lead-store-artifact-prompt.md`, a data grid, not a dashboard.
